...
export function CaseStudyDetailClient({ study, relatedStudies }: CaseStudyDetailClientProps) {
  ...
  const handleCommentSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const response = await fetch(`/api/case-studies/${study.id}/comments`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ userName: commentName, content: commentContent }),
      });
      const data = await response.json();
      if (data.success) {
