...
export default function LoginPage() {
  ...
  const handleSignIn = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (data.success) {
        // Store auth states in localStorage for persistence across pages
        localStorage.setItem("texawave_token", data.token);
        localStorage.setItem("texawave_role", data.role);
        localStorage.setItem("texawave_username", data.name);
        ...
        if (data.role === "admin") {
          ...
          setErrorMsg("Unauthorized role type. Access Denied.");
        }
      } else {
        setErrorMsg(data.error || "Invalid username or password.");
      }
    } catch (err) {
      setErrorMsg("Failed to reach server. Please check your connection.");
    } finally {
      setIsLoading(false);
    }
  };
  ...
  const handleForgotPassword = () => {
    ...
    setErrorMsg("");
    setForgotPasswordMsg(`A reset link has been dispatched to ${email} (Simulated).`);
  };
      ...
      <div className="relative z-10 w-full max-w-5xl bg-bg-primary/60 border border-white/10 rounded-3xl overflow-hidden shadow-premium flex flex-col md:grid md:grid-cols-[1.1fr_1.3fr] min-h-[550px] backdrop-blur-md">
        ...
        <div className="bg-gradient-to-br from-[#080808] to-[#111111] p-8 md:p-12 flex flex-col justify-between border-b md:border-b-0 md:border-r border-white/10 text-left relative overflow-hidden">
          ...
          <div className="text-xs text-text-secondary">
            &copy; {new Date().getFullYear()} Texawave Technologies. All rights reserved.
          </div>
        </div>
        ...
        <div className="p-8 md:p-12 flex flex-col justify-center text-left">
          <div className="mb-8">
            <h2 className="text-2xl font-bold text-white font-display">Staff Portal Login</h2>
            ...
            <div>
              ...
              <div className="relative flex items-center">
                ...
                <button
                  ...
                  className="absolute right-4 text-text-secondary hover:text-white transition-colors p-1"
                >
          ...
          <div className="mt-8 p-3.5 bg-white/5 border border-white/5 rounded-xl text-left">
            ...
            <div className="text-[10px] text-text-secondary space-y-0.5 leading-relaxed font-mono">
              <p>• <strong className="text-white">Admin:</strong> admin@texawave.com / adminpassword</p>
              <p>• <strong className="text-white">HR:</strong> hr@texawave.com / hrpassword</p>
            </div>
          </div>
          ...
          <div className="mt-8 text-center">
            <Link
              href="/careers"
              className="text-xs text-text-secondary hover:text-white transition-colors inline-flex items-center gap-1 font-semibold"
            >
