...
export async function POST(
  ...
  try {
    ...
    studies[index].likes = (studies[index].likes || 0) + 1;
    writeCaseStudies(studies);

    return NextResponse.json({ success: true, likes: studies[index].likes });
  } catch (error: any) {
    return NextResponse.json(
      { success: false, error: error.message || "Failed to update likes" },
      { status: 500 }
    );
  }
}
